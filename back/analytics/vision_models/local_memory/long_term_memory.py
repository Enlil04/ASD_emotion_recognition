...
class LongTermMemoryStore:
    def __init__(self, db_path):
        ...
        # No _create_tables() call here! We rely on setup_db.py
    ...
    def log_significant_event(self, user_id, event_type, description, context=None):
        ...
        conn = self._get_conn()
        try:
            # Uses 'timestamp' to match setup_db.py
            conn.execute("""
    ...
    def add_emotion_counts(self, user_id, date_str, new_counts):
        ...
        try:
            cursor = conn.cursor()
            cursor.execute("SELECT emotion_counts, total_frames FROM emotion_daily WHERE user_id=? AND date_str=?", (user_id, date_str))
            row = cursor.fetchone()
