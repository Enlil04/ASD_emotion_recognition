...
# --- 1. SETUP PATHS FIRST (CRITICAL STEP) ---
...
class MemoryManager:
    ...
    def __init__(self, db_path: str = "memory.db", user_id: str = "user_001"):
        ...
        # No table creation here. We rely on setup_db.py.

    def _connect(self) -> sqlite3.Connection:
        if not os.path.exists(self.db_path):
            print(f"⚠️ WARNING: Database not found at {self.db_path}. Run setup_db.py first!")
    ...
    def save_profile(self, name: Optional[str] = None, triggers: Optional[List[str]] = None,
        ...
        if name is not None: prefs["name"] = name
        if triggers is not None: prefs["triggers"] = triggers
        if preferences is not None: prefs["preferences"] = preferences
