...
# --- SINGLE SOURCE OF TRUTH FOR DB PATHS + SESSION DEP ---
from setup_db import Base, engine, get_db, DB_PATH, DATA_DIR

# --- MODELS (supports both repo layouts) ---
try:
...
# -----------------------------------------------------------------
# HELPERS (weekly chart + recommendation)
# -----------------------------------------------------------------
...
def _fetch_emotion_daily(user_id: str) -> dict:
    ...
    # Convert counts -> per-day proportions (0..1)
    series: dict[str, list[float]] = {}
    for emo, per_day in series_counts.items():
        series[emo] = [(per_day[d] / totals[d]) if totals[d] > 0 else 0.0 for d in days]
...
def _fetch_recent_emotion_from_logs(user_id: str, hours: int = 72) -> Optional[str]:
...
async def daily_recommendation(user_id: str = "user_001"):
    ...
    if brain is None:
        return {"date": today_str, "mode": "error", "dominant": "Neutral", "recommendation": "Brain not ready."}

    week = _fetch_emotion_totals_last_7_days(user_id)
    weekly_total = int(week.get("total", 0))

    recent = _fetch_recent_emotion_from_logs(user_id, hours=72)
    ...
    if weekly_total > 0:
        ...
        stats_text = (
            ...
            f"- Totals: {week['totals']}\n"
            f"- Dominant emotion over the week: {week['dominant']}\n"
            f"- Percentages: {week['percent']}"
        )
    elif recent is not None:
        mode = "recent"
        dominant = recent
        stats_text = (
            "No activity recorded in emotion_daily in the last 7 days.\n"
            f"Most recent emotion seen in the last 72 hours: {recent}"
        )
    elif is_new:
        mode = "new_user"
        dominant = "Neutral"
        stats_text = "No emotion history yet (first-time user)."
    else:
        mode = "no_data"
        dominant = "Neutral"
        stats_text = "No usable emotion history found (e.g., DB reset)."
    ...
    if mode in ("new_user", "no_data"):
        prompt = (
            f"Today is {today_str}.\n"
            f"{stats_text}\n\n"
            "Task: Welcome the user briefly, explain that recommendations personalize after a few check-ins, "
        ...
        prompt = (
            ...
            f"Task: Give ONE practical recommendation for today tailored to the chosen dominant emotion: {dominant}. "
            "If Angry/Sad: calming or coping. If Happy: maintain habits + small growth challenge. If Neutral: gentle routine + exploration. "
...
async def analyze_session_endpoint(
    ...
    try:
        ...
        # 1) Process video (CPU-bound / OpenCV)
        result = await run_in_threadpool(video_service.process_session, temp_path)

        # 2) Normalize emotion (confidence threshold)
        ...
        try:
            ...
            # 4) Upsert into daily summary (for charts)
            today_str = date.today().isoformat()
            daily_entry = (
            ...
            if daily_entry:
                daily_entry.count = int(daily_entry.count or 0) + 1
                daily_entry.updated_at = time.time()
            else:
                db.add(
                    models.DailySummary(
                        user_id="user_001",
            ...
            db.commit()
            print(f"✅ Saved log + daily stats: {emotion} ({confidence:.2f})")
        ...
        system_state["face_detected"] = bool(result.get("face_detected", True))
...
# #if the user has no previous data
...
# Most recent emotion seen in the last 72 hours: {recent}
# """.strip()
#     elif is_new:
#         mode = "new_user"
#         dominant = "Neutral"
#         stats_text = """
# No emotion history yet (first-time user).
# """.strip()
#     else:
#         # Edge case: not "new", but no recent logs + no weekly counts (e.g. DB reset)
#         mode = "no_data"
#         dominant = "Neutral"
#         stats_text = """
# No usable emotion history found.
# """.strip()
